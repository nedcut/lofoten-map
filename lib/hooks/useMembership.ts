 ...
 * No-ops in demo mode (no backend client).
 */
export function useMembership({ backend, trip, loadData }: Options) {
  ...
  const runMemberOperation = useCallback(
    ...
    [backend, trip, loadData, setSaving, reset, setError, setInfo],
  );

  const grantMember = useCallback(
    (input: { email: string; role: MemberRole }) =>
      runMemberOperation(
        () => backend!.rpc("grant_trip_member_by_email", { target_trip_slug: trip!.slug, target_email: input.email, target_role: input.role }),
        `${input.email} added as ${input.role}.`,
      ),
    [runMemberOperation, backend, trip],
  );

  const requestAdmin = useCallback(
    () =>
      runMemberOperation(
        () => backend!.rpc("request_trip_admin", { target_trip_slug: trip!.slug }),
  ...
  const setMemberRole = useCallback(
    (targetUserId: string, role: MemberRole) =>
      runMemberOperation(
        () => backend!.rpc("set_member_role", { target_trip_slug: trip!.slug, target_user_id: targetUserId, new_role: role }),
  ...
  const resolveAdminRequest = useCallback(
    (requestId: string, approve: boolean) =>
      runMemberOperation(
        () => backend!.rpc("resolve_admin_request", { request_id: requestId, approve }),
        approve ? "Request approved." : "Request denied.",
      ),
    [runMemberOperation, backend],
  );
